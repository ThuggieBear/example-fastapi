...
#db: Session = Depends(get_db) grants access to database

# List[] for grabbing more than 1 post. schema is built for 1
@router.get("/", response_model=List[schemas.PostOut])
def get_posts(db: Session = Depends(get_db), current_user:
        int = Depends(oauth2.get_current_user), limit: int = 10, skip: int = 0,
        search: Optional[str] = ""):
...
#current_user: int = Depends(oauth2...) will authenticate user providing token
# first runs through get_current_user function which runs to verify_access_token function
def create_posts(post: schemas.PostCreate, db: Session = Depends(get_db), current_user:
        int = Depends(oauth2.get_current_user)):
    # %s sanitizes inputs.  avoids SQL injection
...
def get_post(id: int, response: Response, db: Session = Depends(get_db), current_user:
        int = Depends(oauth2.get_current_user)):
